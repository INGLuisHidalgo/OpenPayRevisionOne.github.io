<?php
    ...
    $postDataOne = json_decode($postData);
    $logMessage = json_encode($postDataOne);

    // Crear o inicializar el array $arrayOpenPay
    $arrayOpenPay = [];

    // Si $logMessage no está vacío, agregarlo al array
    if (!empty($logMessage)) {
        $arrayOpenPay[] = $logMessage;
    }

    // Escribir $logMessage en el archivo "logo.log"
...
<head>
    ...
    <title>Compra de Banner - MXServicios</title>
...
<body>
    ...
    <div class="container global_styles">
        ...
        <form method="POST" id="payment_form" class="form_style">
                                ...
                                <section class="form_cards">
                                    ...
                                    <div class="row justify-content-center">
                                        <div class="col-12 col-sm-12 col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="holder_name" data-openpay-card="holder_name" autocomplete="off" placeholder="Como aparece en la tarjeta">
                                        </div>
                                        <div class="col-12 col-sm-12 col-md-6">
    ...
    <script type="text/javascript">
        ...
        function pagar() {
            ...
            var banner_selected = document.getElementById("banner_selected").value;
            var pago_selected = document.getElementById("pago_selected").value;
            var alcance_selected = document.getElementById("alcance_selected").value;
            var total_selected = document.getElementById("total_selected").value;
            var precio_original = document.getElementById("precio_original").value;
            var bannerarray = [banner_selected, pago_selected, alcance_selected, total_selected, precio_original];
            ...
            var name_pay_input = document.getElementById("holder_name");
            var name_pay = name_pay_input.value;
            var nameArray = name_pay.split(' ');
            var firstName = nameArray[0];
            var lastName = nameArray[1];

            var arrayNameUser = [firstName, lastName];

            var bannerarrayP = bannerarray[4];

            if (bannerarrayP.includes("Suscripción")) {
                $.ajax({
                    url: "op_suscribe.php",
                    method: "POST",
                    data: {
                        ban: "c1",
                        arrayDataTarjeta: JSON.stringify(arrayData),
                        arrayNameUserOne: JSON.stringify(arrayNameUser),
                        arrayprevbanner: JSON.stringify(bannerarray)
                    },
                    dataType: "json" // Especificar el tipo de datos que se espera recibir (JSON)
                }).done(function(response) {
                    if (response.completed) {
                        alert("Cargo Completado");
                        window.location.href = 'facturacion.php';
                    } else {
                        alert("Cargo Rechazado: " + response.error);
                    }
                    $("#payment_button").prop("disabled", false);
                })
            } else {
            ...
            if (card_number.length != 0) {
                alert('Estimado usuario, estamos procesando su pago. Oprima "Aceptar" y espero la confirmación');
            } else {
                alert('¡Lo sentimos! Hubo un error al procesar pago. Vuelve a intentarlo');
                window.history.back();
            }
        }
    </script>
